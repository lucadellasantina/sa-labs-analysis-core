classdef PropertyDescriptor < matlab.mixin.SetGet %#ok<*MCSUP>
    ...
    methods
        ...
        function t = get.type(obj)
            ft = obj.field.Type;
            t = sa_labs.analysis.core.PropertyType(ft.PrimitiveType, ft.Shape, ft.Domain);
        end
    ...
    methods (Static)
        ...
        function obj = loadobj(s)
            if isstruct(s)
                obj = sa_labs.analysis.core.PropertyDescriptor(s.name, s.value, ...
                    'type', s.type, ...
                    'category', s.category, ...
        ...
        function obj = fromProperty(handle, property)
            ...
            obj = sa_labs.analysis.core.PropertyDescriptor(mpo.Name, handle.(mpo.Name), ...
                'description', comment, ...
                'isReadOnly', mpo.Constant || ~strcmp(mpo.SetAccess, 'public') || mpo.Dependent && isempty(mpo.SetMethod));

            mto = findprop(handle, [property 'Type']);
            if ~isempty(mto) && mto.Hidden && isa(handle.(mto.Name), 'sa_labs.analysis.core.PropertyType')

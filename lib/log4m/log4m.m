    ...
    methods (Static)
        function obj = getLogger( logPath )
            ...
            if(nargin == 0)
                ...
                logdir = [char(home) filesep 'analysis-logs'];
                
                if ~ exist(logdir, 'file')
                    mkdir(logdir);
                end
                fname = ['log4m-' datestr(datetime) '.log'];
                logPath = [logdir filesep strrep(fname, ':', '-')];
